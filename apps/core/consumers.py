...
def connect_room(message, pk):
    ...
    log.debug('chat connect video=%s client=%s:%s', 
        video.pk, message['client'][0], message['client'][1])
...
def receive_room(message, pk):
    ...
    if data:
        log.debug('chat message video=%s user=%s message=%s', 
            video.pk, data['user'], data['message'])
        user = User.objects.get(id=data['user'])
        m = Message.objects.create(video=video, user=user, message=data['message'])
        Group(video.group_name).send({'text': json.dumps({"user": m.user.username, "message": m.message, "created": m.created.strftime("%a %d %b %Y %H:%M")})})
